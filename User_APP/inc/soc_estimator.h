 ...
 * 静置 OCV：充电只向上修剩余容量，放电只向下修；空/满电需稳定后再夹紧。
 * 满充容量学习：learned_cap_pct ≈ 实测满充容量 / 标称（见 SOC_LEARN_CHARGE_* 宏）。
...
typedef struct
{
  ...
  bool soc_valid;
  int32_t soc_remaining_mah;    /* 估算剩余容量 */
  int32_t soc_nominal_mah;      /* 标称容量 */
  uint8_t learned_cap_pct;      /* 0~100，满充学习容量/标称（非 SohEstimator） */
  bool learned_cap_valid;       /* 至少完成一次可信满充容量学习 */
} soc_status_t;
