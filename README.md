...
| 被动均衡 | `User_APP/src/cell_balance_manager.c` | 顶部细均衡 + 中段最高芯保护 / 真伪鉴别 |
...
- **顶部才认真拉齐**（SOC ≥ 90% 或无 SOC 时 vmin ≥ 3900 mV，Δ 40/15 mV）
- **中段第一目标是护最高芯、确认是不是真不平衡**；拉齐是第二目标，被动均衡不在中段追齐
...
| `CHARGE_IMBALANCE_STOP_DELTA_MV` | 300 | **仅顶部** Δ≥300 → 停充（台架；量产改回 50） |
| `CHARGE_IMBALANCE_RESUME_DELTA_MV` | 250 | **仅顶部** Δ≤250 → 恢复（台架；量产改回 30） |
| `BALANCE_MID_VMAX_DANGER_MV` | 4120 | 中段：最高芯进入保护 |
| `BALANCE_MID_VMAX_SAFE_MV` | 4080 | 中段：最高芯离开危险，恢复充电 |
| `BALANCE_MID_OBSERVE_DELTA_MV` | 150 | 中段：本周期做一次停充松弛鉴别 |
| `BALANCE_MID_RELAX_COUNT` | 6 | 松弛约 3 s（500 ms 拍） |

### 中段：护最高芯 + 真伪鉴别（不拉齐）

未进入顶部窗口时，被动均衡**默认关闭**。中段不把 Δ 收到 15 mV；成功标准是最高芯离开危险区，以及分清 IR 假压差和真容量差。

```
充电中、未到顶部
  → 本周期 Δ ≥ 150 mV，或 vmax ≥ 4120 mV
  → 停充约 3 s（松弛，IR 落下）
  → 静置后三分：
       A. Δ 明显缩小或已不大     → 假不平衡（内阻），恢复充电，不泄放
       B. Δ 仍大，但 vmax < 4120 → 真不平衡但不危险，恢复充电，不泄放
       C. vmax 仍 ≥ 4120         → 真威胁：只泄最高 1~2 节，直到 vmax ≤ 4080
  → 同一充电周期对「大压差观察」只做一次，避免 IR 压差反复停充
  → vmax 再次顶到 4120 仍可再进入保护（约 5 s 冷却）
```

中段泄放选节：只选仍处于高位带的最高芯（`vmax − 20 mV` 且 ≥ 4000 mV），不是 `vmin + 15 mV`。最长约 30 s，超时也恢复充电，避免卡死。

拉齐仍留给顶部被动均衡；中段认真搬能量需要主动均衡（本工程无此硬件）。

### 顶部压差停充（与 40/15 细均衡独立）

仅在顶部窗口：Δ 过大时先停充，顶部被动均衡可继续，压差缩小后再恢复充电。中段**不用**这组 Δ 门槛停充。

```
顶部窗口
  → Δ ≥ 300 mV（台架；量产 50）：置位压差停充，拉高 CFETOFF
  → 仅保留顶部被动均衡（不因 CHG FET 关闭而退出）
  → Δ ≤ 250 mV（台架；量产 30）：清除压差停充，恢复充电
  → Δ ≤ 15 mV：顶部均衡完成
```
...
| 顶部停止充电的压差 | **≥ 300 mV**（台架；量产 50） |
| 顶部恢复充电的压差 | **≤ 250 mV**（台架；量产 30） |
| 中段停充 | 松弛鉴别（Δ≥150 一次）或 vmax≥4120 |
| 中段恢复 | 鉴别结束且非威胁，或保护后 vmax≤4080 |
...
| `ACTIVE` | 顶部细均衡（≤2 节） |
| `INHIBITED` | 安全 / 故障禁止 |
| `MID_RELAX` | 中段停充松弛，鉴别真假不平衡 |
| `MID_PROTECT` | 中段短时泄最高芯，护 vmax |

`mid_class`：`NONE` / `FAKE_IR`（假压差）/ `REAL_SAFE`（真但不危险）/ `REAL_THREAT`（最高芯仍顶在上限）。
